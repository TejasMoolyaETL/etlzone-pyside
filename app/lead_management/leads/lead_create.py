...
class CreateLeadPage(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        hl.addWidget(QLabel("Create Lead"))
        hl.addStretch()
        ...
        core_row += 1
        add_line_field(core_grid, "status", core_row, 0, col_span=2)
        core_row += 1
    ...
    def _refresh_create_access(self) -> None:
        ...
        if steps is None:
            ...
            self._can_create = nav_action_visible("Leads", "create", allowed)
    ...
    def reset_to_default(self) -> None:
        ...
        if self._job_desc_edit is not None:
            self._job_desc_edit.clear()
            self._job_desc_edit.setMinimumHeight(self._job_desc_default_min_h)
            host = self._job_desc_edit.parent()
            if isinstance(host, _JobDescEditorWithGrip):
                host.setMinimumHeight(self._job_desc_default_min_h)
    ...
    def _handle_create(self) -> None:
        if not self._can_create:
            self._show_error("Create disabled: missing step access lead-create.")
            return
        self._clear_error()
        payload = self._payload_from_form()
        if payload is None:
            return
        result = api_create_lead(payload, token=self._token())
        ok = show_api_result_message(
            self,
            self.error_label,
            result,
            error_fallback="Failed to create lead.",
            success_fallback="Lead created successfully.",
        )
        if ok:
