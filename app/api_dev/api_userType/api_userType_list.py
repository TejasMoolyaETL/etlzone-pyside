"""API User Involved list — same layout and behavior as API: Projects list."""
...
from core.api import api_delete_user_type_in_api_project_by_id, api_get_all_user_type_in_api_project
...
class _UserInvolvedLoadWorker(QObject):
    ...
    def run(self) -> None:
        try:
            result = api_get_all_user_type_in_api_project(token=self._token)
        except Exception as exc:
            self.finished.emit(False, [], f"Load failed ({type(exc).__name__}).")
            return
        if not result.get("success"):
            self.finished.emit(False, [], str(result.get("message", "Failed to load user involved records.")))
            return
...
class APIUserInvolvedPage(QWidget):
    """API → API: User Involved — matches API: Projects list UX."""
    ...
    def __init__(
        ...
        self._load_worker: _UserInvolvedLoadWorker | None = None
    ...
    def _build_ui(self) -> None:
        ...
        title = QLabel("API: User Involved")
    ...
    def _refresh_action_access(self) -> None:
        ...
        if steps is None:
            ...
            self._can_create = nav_action_visible("API: User Involved", "create", allowed_actions)
            self._can_display = nav_action_visible("API: User Involved", "display", allowed_actions)
            self._can_edit = nav_action_visible("API: User Involved", "edit", allowed_actions)
            self._can_delete = nav_action_visible("API: User Involved", "delete", allowed_actions)
    ...
    def _populate_table(self, rows: list[dict[str, Any]]) -> None:
        ...
        try:
            ...
            self._column_spec = list(_USER_INVOLVED_COLUMN_SPEC)
            self.table.setSortingEnabled(False)
    ...
    def refresh(self) -> None:
        ...
        self._load_worker = _UserInvolvedLoadWorker(token)
    ...
    def _on_table_context_menu(self, pos: QPoint) -> None:
        ...
        add_action = menu.addAction("Add Role")
        display_action = menu.addAction("Display Selected Role")
        edit_action = menu.addAction("Edit Selected Role")
        remove_action = menu.addAction("Remove Selected Role")
        ...
        display_action.setEnabled(has_selected and self.on_edit_clicked is not None and self._can_display)
        edit_action.setEnabled(has_selected and self.on_edit_clicked is not None and self._can_edit)
        remove_action.setEnabled(has_selected and self._can_delete)
    ...
    def _edit_selected(self) -> None:
        ...
        if _internal_row_id(rec) is None:
            self._show_message("Cannot edit: missing internal id.", error=True)
            return
        if self.on_edit_clicked:
    ...
    def _remove_selected(self) -> None:
        ...
        if internal_id is None:
            self._show_message("Cannot remove: internal id is missing.", error=True)
            return
        role = (
            (rec.get("userType") or rec.get("user_type") or "") or ""
        ).strip().upper()
        if not role:
            self._show_message("Cannot remove: role is missing.", error=True)
            return
        reply = QMessageBox.question(
            self,
            "Remove Role",
            f"Remove role '{role}' from this project assignment?",
        ...
        if not token:
            ...
            return
        result = api_delete_user_type_in_api_project_by_id(internal_id, token=token)
        if result.get("success"):
            self._show_message(result.get("message", "Role removed."), error=False)
            self.refresh()
            return
        self._show_message(result.get("message", "Failed to remove role."), error=True)
