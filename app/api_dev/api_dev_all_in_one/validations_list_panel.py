"""Replica of API Validations list for API: All in One only (no import from ``api_validations`` package)."""
...
from core.api import api_delete_api_validation_by_id, api_get_all_api_validations
...
_API_VALIDATION_LIST_COLUMN_SPEC: tuple[tuple[str, tuple[str, ...]], ...] = tuple(
    (label, keys) for label, keys in _API_VALIDATION_COLUMN_SPEC if label != "Project Id"
)
...
class _APIValidationsLoadWorker(QObject):
    ...
    def run(self) -> None:
        try:
            result = api_get_all_api_validations(token=self._token)
        except Exception as exc:
            self.finished.emit(False, [], f"Load failed ({type(exc).__name__}).")
            return
        if not result.get("success"):
            self.finished.emit(False, [], str(result.get("message", "Failed to load API validations.")))
            return
...
class APIValidationsListPanel(QWidget):
    """Same behavior as ``APIValidationsPage``; duplicated here for All in One isolation."""
    ...
    def __init__(
        ...
        *,
        ...
        self._load_worker: _APIValidationsLoadWorker | None = None
    ...
    def _refresh_action_access(self) -> None:
        ...
        if steps is None:
            ...
            self._can_create = nav_action_visible("API: Validations", "create", allowed_actions)
            self._can_display = nav_action_visible("API: Validations", "display", allowed_actions)
            self._can_edit = nav_action_visible("API: Validations", "edit", allowed_actions)
            self._can_delete = nav_action_visible("API: Validations", "delete", allowed_actions)
    ...
    def _populate_table(self, rows: list[dict[str, Any]]) -> None:
        ...
        try:
            ...
            self._column_spec = list(_API_VALIDATION_LIST_COLUMN_SPEC)
            self.table.setSortingEnabled(False)
    ...
    def refresh(self) -> None:
        ...
        self._load_worker = _APIValidationsLoadWorker(token)
    ...
    def get_selected_row(self) -> dict[str, Any] | None:
        """Current data row selection (for All in One comment panel)."""
    ...
    def _on_table_context_menu(self, pos: QPoint) -> None:
        ...
        add_action = menu.addAction("Add API Validation")
        display_action = menu.addAction("Display Selected API Validation")
        edit_action = menu.addAction("Edit Selected API Validation")
        remove_action = menu.addAction("Remove Selected API Validation")
    ...
    def _edit_selected(self) -> None:
        ...
        if _internal_row_id(rec) is None:
            self._show_message("Cannot edit: missing internal validation id.", error=True)
            return
        if self.on_edit_clicked:
    ...
    def _remove_selected(self) -> None:
        ...
        if internal_id is None:
            self._show_message("Cannot remove: internal validation id is missing.", error=True)
            return
        field_name = (
            (rec.get("fieldName") or rec.get("field_name") or rec.get("apiValidation") or rec.get("api_validation") or "")
        ).strip()
        display_name = field_name or str(internal_id)
        reply = QMessageBox.question(
            self,
            "Remove API Validation",
            f"Remove API validation '{display_name}'?",
        ...
        if not token:
            ...
            return
        result = api_delete_api_validation_by_id(internal_id, token=token)
        if result.get("success"):
            self._show_message(result.get("message", "API validation removed."), error=False)
            self.refresh()
            return
        self._show_message(result.get("message", "Failed to remove API validation."), error=True)
