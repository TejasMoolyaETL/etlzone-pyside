"""Roles list — GET roles/get-all-roles, same table UX as Department list."""
...
from core.api import api_delete_role, api_get_all_roles
...
def _flatten_role(row: dict[str, Any]) -> dict[str, Any]:
...
class RolesListPage(QWidget):
    """Fetches and displays roles in a table; double-click / context menu open view/edit."""

    def __init__(
        self,
        on_create_clicked: Callable[[], None] | None = None,
        on_view_clicked: Callable[[dict[str, Any]], None] | None = None,
        on_edit_role: Callable[[dict[str, Any]], None] | None = None,
    ) -> None:
        super().__init__()
        self.on_create_clicked = on_create_clicked
        self.on_view_clicked = on_view_clicked
        self.on_edit_role = on_edit_role
    ...
    def _populate_table(self, rows: list[dict[str, Any]]) -> None:
        ...
        try:
            ...
            self._column_spec = list(_ROLE_COLUMN_SPEC)
            self.table.setSortingEnabled(False)
            ...
            show_auto_hiding_message(
                self, self._message_label, "Failed to display roles.", error=True
            )
            self._show_empty_table()
    ...
    def _load_roles(self) -> None:
        ...
        if not result.get("success"):
            show_auto_hiding_message(
                self,
                self._message_label,
                result.get("message", "Failed to load roles."),
                error=True,
            )
            self._show_empty_table()
            return
    ...
    def _on_item_double_clicked(self, item: QTableWidgetItem) -> None:
        ...
        if row_item is None:
            return
        role = row_item.data(Qt.ItemDataRole.UserRole)
        if isinstance(role, dict):
            self.on_view_clicked(role)

    def _role_at_pos(self, pos: QPoint) -> dict[str, Any] | None:
    ...
    def _get_role_id(self, role: dict[str, Any]) -> int | str | None:
    ...
    def _on_context_menu(self, pos: QPoint) -> None:
        ...
        row = self._role_at_pos(pos)
        menu = QMenu(self)
        menu.setStyleSheet(CONTEXT_MENU_STYLESHEET)
        add_action = menu.addAction("Add Role")
        display_action = menu.addAction("Display Role")
        ...
        edit_action.setEnabled(has_data and self.on_edit_role is not None)
        delete_action.setEnabled(has_data)

        action = menu.exec(QCursor.pos())
        target = self._selected_role_row() or row
        ...
        elif action == edit_action and target and self.on_edit_role:
            self.on_edit_role(target)
        elif action == delete_action and target:
            self._handle_delete_role(target)

    def _handle_delete_role(self, role: dict[str, Any]) -> None:
        rid = self._get_role_id(role)
        if rid is None:
            show_auto_hiding_message(
                self, self._message_label, "Cannot delete: Role ID is missing.", error=True
            )
            return
        name = role.get("roleName") or role.get("role_name") or str(rid)
        ...
        token = str(token) if token else None
        result = api_delete_role(rid, token=token)
        if result.get("success"):
            show_auto_hiding_message(
                self,
                self._message_label,
                result.get("message", "Role deleted."),
                error=False,
            )
            self.refresh()
        else:
            show_auto_hiding_message(
                self,
                self._message_label,
                result.get("message", "Failed to delete role."),
                error=True,
            )
