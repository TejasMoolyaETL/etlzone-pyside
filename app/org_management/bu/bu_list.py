"""Business unit list — GET api/bu/get-all-bu, same UX as Organization list."""
...
def _flatten_bu(row: dict[str, Any]) -> dict[str, Any]:
...
def _value_for_column(row: dict[str, Any], keys: tuple[str, ...]) -> tuple[Any, str]:
    ...
    if keys and keys[0] in ("organizationName", "organization_name", "orgName", "org_name"):
        ...
        oname = get_organization_name_from_bu(row)
        if oname:
        ...
        for key in keys:
            if key in flat:
                ...
                if isinstance(v, dict):
                    continue
                if v is not None and str(v).strip():
                    return (str(v).strip(), "parentBuName")
        pname = get_parent_bu_name_from_bu(row)
        if pname:
...
class BuListPage(QWidget):
    ...
    def _populate_table(self, rows: list[dict[str, Any]]) -> None:
        ...
        try:
            ...
            self._column_spec = list(_BU_COLUMN_SPEC)
            self.table.setSortingEnabled(False)
            ...
            show_auto_hiding_message(
                self, self._message_label, "Failed to display business units.", error=True
            )
            self._show_empty_table()
    ...
    def _load_bus(self) -> None:
        ...
        if not result.get("success"):
            show_auto_hiding_message(
                self,
                self._message_label,
                result.get("message", "Failed to load business units."),
                error=True,
            )
            self._show_empty_table()
            return
    ...
    def _selected_bu_row(self) -> dict[str, Any] | None:
    ...
    def _on_item_double_clicked(self, item: QTableWidgetItem) -> None:
        ...
        if row_item is None:
            return
        bu = row_item.data(Qt.ItemDataRole.UserRole)
        if isinstance(bu, dict):
            self.on_view_clicked(bu)

    def _bu_at_pos(self, pos: QPoint) -> dict[str, Any] | None:
        vp = self.table.viewport()

        def _bu_from_viewport_point(vp_point: QPoint) -> dict[str, Any] | None:
            ...
            if idx.isValid():
                ...
                if item:
                    bu = item.data(Qt.ItemDataRole.UserRole)
                    if isinstance(bu, dict):
                        return bu
        ...
        bu = _bu_from_viewport_point(pos)
        if bu is not None:
            return bu
        vp_pos = vp.mapFrom(self.table, pos)
        return _bu_from_viewport_point(vp_pos)

    def _get_bu_id(self, bu: dict[str, Any]) -> int | str | None:
    ...
    def _on_context_menu(self, pos: QPoint) -> None:
        ...
        bu = self._bu_at_pos(pos)
        menu = QMenu(self)
        menu.setStyleSheet(CONTEXT_MENU_STYLESHEET)
        add_action = menu.addAction("Add Business Unit")
        display_action = menu.addAction("Display Business Unit")
        edit_action = menu.addAction("Edit Business Unit")
        delete_action = menu.addAction("Delete Business Unit")
        ...
        edit_action.setEnabled(has_data and self.on_edit_bu is not None)
        delete_action.setEnabled(has_data)
        action = menu.exec(QCursor.pos())
        target = self._selected_bu_row() or bu
        ...
        elif action == edit_action and target and self.on_edit_bu:
            self.on_edit_bu(target)
        elif action == delete_action and target:
            self._handle_delete_bu(target)

    def _handle_delete_bu(self, bu: dict[str, Any]) -> None:
        bu_id = self._get_bu_id(bu)
        if bu_id is None:
            show_auto_hiding_message(
                self, self._message_label, "Cannot delete: Business unit ID is missing.", error=True
            )
            return
        name = bu.get("buName") or bu.get("bu_name") or str(bu_id)
        reply = QMessageBox.question(
            self,
            "Delete Business Unit",
            f"Are you sure you want to delete business unit '{name}'?",
        ...
        token = str(token) if token else None
        result = api_delete_bu(bu_id, token=token)
        if result.get("success"):
            show_auto_hiding_message(
                self,
                self._message_label,
                result.get("message", "Business unit deleted."),
                error=False,
            )
            self.refresh()
        else:
            show_auto_hiding_message(
                self,
                self._message_label,
                result.get("message", "Failed to delete business unit."),
                error=True,
            )
