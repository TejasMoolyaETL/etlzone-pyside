"""Department list — GET departments/get-all-depts, BU-list style UX."""
...
from core.api import api_delete_dept, api_get_all_depts
...
def _flatten_dept(dept: dict[str, Any]) -> dict[str, Any]:
    return {k: v for k, v in dept.items() if k not in _HIDDEN_KEYS}


def _nested_value(obj: dict[str, Any], *keys: str) -> Any:
    for key in keys:
        val = obj.get(key)
...
def _parent_department_object_from_dept(dept: dict[str, Any]) -> dict[str, Any] | None:
    """Parent dept may be nested as parentDepartment or parent_dept."""
...
def _organization_object_from_dept(dept: dict[str, Any]) -> dict[str, Any] | None:
    """Org may be on the dept root or nested under businessUnit / bu."""
    org = dept.get("organization")
    if isinstance(org, dict):
        return org
    bu = dept.get("businessUnit") or dept.get("bu")
    if isinstance(bu, dict):
        nested = bu.get("organization")
        if isinstance(nested, dict):
            return nested
    return None


def _value_for_column(dept: dict[str, Any], keys: tuple[str, ...]) -> tuple[Any, str]:
    flat = _flatten_dept(dept)

    if keys and keys[0] in ("organizationId", "organization_id", "orgId", "org_id"):
        for key in keys:
            if key in flat and not isinstance(flat[key], dict) and flat[key] is not None:
                return (flat[key], key)
        org = _organization_object_from_dept(dept)
        if isinstance(org, dict):
            v = _nested_value(org, "orgId", "org_id", "organizationId", "id")
            if v is not None:
    ...
    if keys and keys[0] in ("organizationName", "organization_name", "orgName", "org_name"):
        ...
        org = _organization_object_from_dept(dept)
        if isinstance(org, dict):
            v = _nested_value(org, "orgName", "org_name", "name")
            if v is not None and str(v).strip():
    ...
    if keys and keys[0] in ("buName", "bu_name", "businessUnitName", "business_unit_name"):
        ...
        bu = dept.get("businessUnit") or dept.get("bu")
        if isinstance(bu, dict):
            v = _nested_value(bu, "buName", "bu_name", "name")
            if v is not None and str(v).strip():
...
class DeptListPage(QWidget):
    """Fetches and displays departments in a table."""

    def __init__(
        self,
        on_create_clicked: Callable[[], None] | None = None,
        on_view_clicked: Callable[[dict[str, Any]], None] | None = None,
        on_edit_dept: Callable[[dict[str, Any]], None] | None = None,
        on_delete_dept: Callable[[dict[str, Any]], None] | None = None,
    ) -> None:
        super().__init__()
        self.on_create_clicked = on_create_clicked
        self.on_view_clicked = on_view_clicked
        self.on_edit_dept = on_edit_dept
        self.on_delete_dept = on_delete_dept
    ...
    def _populate_table(self, rows: list[dict[str, Any]]) -> None:
        ...
        try:
            ...
            self._column_spec = list(_DEPT_COLUMN_SPEC)
            self.table.setSortingEnabled(False)
            ...
            show_auto_hiding_message(
                self, self._message_label, "Failed to display departments.", error=True
            )
            self._show_empty_table()
    ...
    def _load_depts(self) -> None:
        ...
        if not result.get("success"):
            show_auto_hiding_message(
                self,
                self._message_label,
                result.get("message", "Failed to load departments."),
                error=True,
            )
            self._show_empty_table()
            return
    ...
    def _selected_dept_row(self) -> dict[str, Any] | None:
    ...
    def _on_item_double_clicked(self, item: QTableWidgetItem) -> None:
        ...
        if row_item is None:
            return
        dept = row_item.data(Qt.ItemDataRole.UserRole)
        if isinstance(dept, dict):
            self.on_view_clicked(dept)

    def _dept_at_pos(self, pos: QPoint) -> dict[str, Any] | None:
        ...
        def _from_viewport(vp_point: QPoint) -> dict[str, Any] | None:
            ...
            if idx.isValid():
                ...
                if item:
                    dept = item.data(Qt.ItemDataRole.UserRole)
                    if isinstance(dept, dict):
                        return dept
    ...
    def _on_context_menu(self, pos: QPoint) -> None:
        ...
        dept = self._dept_at_pos(pos)
        menu = QMenu(self)
        menu.setStyleSheet(CONTEXT_MENU_STYLESHEET)
        add_action = menu.addAction("Add Department")
        display_action = menu.addAction("Display Department")
        ...
        edit_action.setEnabled(has_data and self.on_edit_dept is not None)
        delete_action.setEnabled(has_data)

        action = menu.exec(QCursor.pos())
        target = self._selected_dept_row() or dept
        ...
        elif action == edit_action and target and self.on_edit_dept:
            self.on_edit_dept(target)
        elif action == delete_action and target:
            if self.on_delete_dept:
                self.on_delete_dept(target)
            else:
                self._handle_delete_dept(target)

    def _get_dept_id(self, dept: dict[str, Any]) -> int | str | None:
        for k in ("deptId", "dept_id", "departmentId", "department_id", "id"):
            v = dept.get(k)
            if v is not None:
                return v
        return None

    def _handle_delete_dept(self, dept: dict[str, Any]) -> None:
        dept_id = self._get_dept_id(dept)
        if dept_id is None:
            show_auto_hiding_message(
                self, self._message_label, "Cannot delete: Department ID is missing.", error=True
            )
            return
        name = dept.get("deptName") or dept.get("dept_name") or str(dept_id)
        reply = QMessageBox.question(
            self,
            "Delete Department",
            f"Are you sure you want to delete department '{name}'?",
        ...
        token = str(token) if token else None
        result = api_delete_dept(dept_id, token=token)
        if result.get("success"):
            show_auto_hiding_message(
                self,
                self._message_label,
                result.get("message", "Department deleted."),
                error=False,
            )
            self.refresh()
        else:
            show_auto_hiding_message(
                self,
                self._message_label,
                result.get("message", "Failed to delete department."),
                error=True,
            )
