"""Dialog to pick imported tables to add to the extraction list."""
...
_TABLE_COLUMN_SPEC: tuple[tuple[str, tuple[str, ...]], ...] = (
    ...
    ("TGT_TABLE_NAME", ("TGT_TABLE_NAME", "tgtTableName", "tgt_table_name", "newTableName")),
...
class ExtractionAddTableDialog(QDialog):
    def __init__(
        self,
        ...
        *,
        ...
        existing_table_names: set[str] | frozenset[str] | None = None,
    ) -> None:
        super().__init__(parent)
        self.setWindowTitle("Add Table to Extraction")
        self.setModal(True)
        self.setMinimumSize(560, 420)

        self._tables = [dict(row) for row in tables if isinstance(row, dict)]
        self._existing = frozenset(existing_table_names or set())
        ...
        title = connection_name.strip() or "Connection"
        hint = QLabel(
            f"Select imported tables to add for \"{title}\". "
            "Tables already in the extraction list are checked and cannot be added again."
        )
    ...
    def _render_table(self) -> None:
        ...
        for row_idx, row in enumerate(self._tables):
            ...
            already_added = table_name in self._existing
            check_item = QTableWidgetItem()
            check_item.setFlags(Qt.ItemFlag.ItemIsEnabled | Qt.ItemFlag.ItemIsUserCheckable)
            check_item.setCheckState(
                Qt.CheckState.Checked if already_added else Qt.CheckState.Unchecked
            )
            if already_added:
    ...
    def _sync_select_all(self) -> None:
        ...
        selectable_rows = [
            ...
            and self._table.item(row, 0).flags() & Qt.ItemFlag.ItemIsEnabled
        ]
    ...
    def _on_select_all_changed(self, state: int) -> None:
        ...
        for row in range(self._table.rowCount()):
            ...
            if check_item and check_item.flags() & Qt.ItemFlag.ItemIsEnabled:
    ...
    def newly_checked_table_names(self) -> list[str]:
        names: list[str] = []
        for row in range(self._table.rowCount()):
            check_item = self._table.item(row, 0)
            ...
            if not table_name or table_name in self._existing:
                continue
    ...
    def _on_accept(self) -> None:
        if not self.newly_checked_table_names():
