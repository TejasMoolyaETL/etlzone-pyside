...
from core.api import api_delete_dm_company, api_get_all_dm_companies
...
class DmCompanyListPage(QWidget):
    def __init__(
        self,
    ...
    def _build_ui(self) -> None:
        ...
        hl.addWidget(QLabel("DM: Company"))
        hl.addStretch()
    ...
    def _emit_create(self) -> None:
        if not self._can_create_company:
            show_auto_hiding_message(
                self,
                self._message_label,
                "Create disabled: missing step access dm-company-create.",
                error=True,
            )
            return
    ...
    def _refresh_action_access(self) -> None:
        ...
        if steps is None:
            ...
            self._can_display_company = nav_action_visible("DM: Company", "display", allowed)
            self._can_edit_company = nav_action_visible("DM: Company", "edit", allowed)
            self._can_delete_company = nav_action_visible("DM: Company", "delete", allowed)
    ...
    def _handle_delete_company(self, company: dict[str, Any]) -> None:
        ...
        if reply != QMessageBox.StandardButton.Yes:
            return
        result = api_delete_dm_company(cid, token=self._token())
        show_auto_hiding_message(
            self,
            self._message_label,
