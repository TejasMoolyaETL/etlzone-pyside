...
from core.api import api_delete_dm_contact_person, api_get_all_dm_contact_persons
...
class DmContactPersonListPage(QWidget):
    def __init__(
        self,
    ...
    def _build_ui(self) -> None:
        ...
        hl.addWidget(QLabel("DM: Contact Person"))
        hl.addStretch()
    ...
    def _emit_create(self) -> None:
        if not self._can_create:
            show_auto_hiding_message(
                self,
                self._msg,
                "Create disabled: missing step access dm-contact-person-create.",
                error=True,
            )
            return
    ...
    def _refresh_action_access(self) -> None:
        ...
        if steps is None:
            ...
            self._can_display = nav_action_visible("DM: Contact Person", "display", allowed)
            self._can_edit = nav_action_visible("DM: Contact Person", "edit", allowed)
            self._can_delete = nav_action_visible("DM: Contact Person", "delete", allowed)
    ...
    def _on_context_menu(self, pos: QPoint) -> None:
        ...
        if act == add:
            ...
            if QMessageBox.question(
                ...
                return
            res = api_delete_dm_contact_person(pid, token=self._token())
