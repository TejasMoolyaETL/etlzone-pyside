"""View / edit Module page — same shell as Organization Details."""
...
def _get_value(mod: dict[str, Any], keys: tuple[str, ...]) -> Any:
    flat = {k: v for k, v in mod.items() if k not in _HIDDEN_KEYS}
    for key in keys:
        if key in flat:
            return flat[key]
    return None
...
def _get_module_id(module: dict[str, Any]) -> int | str | None:
    for k in ("moduleId", "id"):
        v = module.get(k)
        if v is not None:
            return v
    return None
...
_MODULE_FIELD_GROUPS = _MODULE_COL1 + _MODULE_COL2
...
class ViewModulePage(QWidget):
    """Page that displays module details with view and edit modes."""
    ...
    def _build_ui(self) -> None:
        ...
        for idx, (label_text, keys) in enumerate(_MODULE_COL1):
    ...
    def _refresh_values(self) -> None:
        for _label_text, keys in _MODULE_FIELD_GROUPS:
            canonical = keys[0]
            value = _get_value(self._module, keys)
    ...
    def _has_unsaved_changes(self) -> bool:
        for _label_text, keys in _MODULE_FIELD_GROUPS:
            ...
            if canonical not in self._editable_keys:
                continue
            original = _get_value(self._module, keys)
    ...
    def _handle_save(self) -> None:
        ...
        if not self._has_unsaved_changes():
            ...
            return
        name = self._get_edit_value("moduleName", "name")
        if not name:
            self._show_error("Module name is required.")
            name_edit = self._field_edits.get("moduleName")
            if name_edit:
                name_edit.setFocus()
            return
        module_id = _get_module_id(self._module)
