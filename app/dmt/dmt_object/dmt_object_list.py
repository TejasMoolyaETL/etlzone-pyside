"""DMT - Object list page for DMT Tracker."""
...
from core.api import api_delete_object, api_get_all_objects
...
_OBJECT_COLUMN_SPEC: tuple[tuple[str, tuple[str, ...]], ...] = (
    ("Object Id", ("objectId", "id")),
    ("Object Name", ("objectName", "name")),
    ("Module Id", ("moduleId", "module_id")),
    ("Module Name", ("moduleName", "module_name")),
    ("Status", ("statusName", "statusLabel", "status", "statusSeq", "status_seq", "dmt_object_status")),
...
def _object_value_for_column(row: dict[str, Any], keys: tuple[str, ...]) -> tuple[Any, str]:
    for key in keys:
...
class DmtObjectListPage(QWidget):
    def __init__(
        self,
    ...
    def _build_ui(self) -> None:
        ...
        hl.addWidget(QLabel("DMT - Object"))
        hl.addStretch()
    ...
    def _filtered_source_rows(self) -> list[dict[str, Any]]:
        return filter_dict_rows_by_column_edits(
            self._source_rows,
            self.table,
            list(_OBJECT_COLUMN_SPEC),
            self._filter_visible,
            _object_value_for_column,
            _format_cell,
        )
    ...
    def _write_data_rows(self, rows: list[dict[str, Any]]) -> None:
        ...
        for r, row in enumerate(rows):
            tr = off + r
            for col, (_, keys) in enumerate(_OBJECT_COLUMN_SPEC):
                value, key_used = _object_value_for_column(row, keys)
    ...
    def _get_object_at_row(self, row: int) -> dict[str, Any] | None:
    ...
    def _object_at_pos(self, pos: QPoint) -> dict[str, Any] | None:
        idx = self.table.indexAt(pos)
        if idx.isValid():
            return self._get_object_at_row(idx.row())
        item = self.table.itemAt(pos)
        if item:
            return self._get_object_at_row(item.row())
        return None

    def _on_context_menu(self, pos: QPoint) -> None:
        obj = self._object_at_pos(pos)
        menu = QMenu(self)
        menu.setStyleSheet(CONTEXT_MENU_STYLESHEET)
        add_action = menu.addAction("Add Object")
        display_action = menu.addAction("Display Object")
        edit_action = menu.addAction("Edit Object")
        delete_action = menu.addAction("Delete Object")
        display_action.setEnabled(obj is not None and self.on_edit_clicked is not None)
        edit_action.setEnabled(obj is not None and self.on_edit_clicked is not None)
        delete_action.setEnabled(obj is not None)
        action = menu.exec(QCursor.pos())
        if action == add_action and self.on_create_clicked:
            self.on_create_clicked()
        elif action == display_action and obj is not None and self.on_edit_clicked:
            self.on_edit_clicked(obj, False)
        elif action == edit_action and obj is not None and self.on_edit_clicked:
            self.on_edit_clicked(obj, True)
        elif action == delete_action and obj is not None:
            self._handle_delete_object(obj)

    def _on_row_double_clicked(self, item: QTableWidgetItem) -> None:
        if item.row() < self._data_row_offset():
            return
        obj = self._get_object_at_row(item.row())
        if obj is not None and self.on_edit_clicked:
            self.on_edit_clicked(obj, False)

    def _handle_delete_object(self, obj: dict[str, Any]) -> None:
        oid = obj.get("objectId") or obj.get("id")
        if oid is None:
            show_auto_hiding_message(self, self._message_label, "Cannot delete: Object ID is missing.", error=True)
            return
        name = obj.get("objectName") or obj.get("name") or "this object"
        ...
        result = api_delete_object(oid, token=token)
        if result.get("success"):
            show_auto_hiding_message(self, self._message_label, str(result.get("message") or "Object deleted."), error=False)
            QTimer.singleShot(700, self._load_objects)
        else:
            show_auto_hiding_message(self, self._message_label, str(result.get("message") or "Failed to delete object."), error=True)
    ...
    def _load_objects(self) -> None:
        ...
        token = str(token) if token else None
        result = api_get_all_objects(token=token)
        if not result.get("success"):
            show_auto_hiding_message(self, self._message_label, str(result.get("message") or "Failed to load objects."), error=True)
            self._show_empty_table()
            return
        rows = result.get("data") or []
        valid_rows = [dict(r) for r in rows if isinstance(r, dict)]
        self._source_rows = valid_rows
        self.table.setColumnCount(len(_OBJECT_COLUMN_SPEC))
        self.table.setHorizontalHeaderLabels([h for h, _ in _OBJECT_COLUMN_SPEC])
