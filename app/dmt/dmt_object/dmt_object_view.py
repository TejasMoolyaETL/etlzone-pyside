"""View / edit DMT Object page — same shell as Module Details."""
...
from core.api import api_update_object, master_key_row_seq_value
...
def _status_seq_from_record(rec: dict[str, Any]) -> Any | None:
    for key in ("statusSeq", "status_seq", "status", "dmt_object_status"):
        v = rec.get(key)
        if v is None:
            continue
...
def _get_object_id(rec: dict[str, Any]) -> int | str | None:
...
_OBJECT_FIELD_GROUPS = _OBJECT_COL1 + _OBJECT_COL2

_READONLY_KEYS = frozenset({
    "objectId", "moduleId", "module_id", "moduleName", "module_name",
...
class ViewDmtObjectPage(QWidget):
    def __init__(
        self,
    ...
    def _build_ui(self) -> None:
        ...
        def add_field(col: int, idx: int, label_text: str, keys: tuple[str, ...]) -> None:
            ...
            value_edit.setText("")
            if canonical == "objectName":
                value_edit.setPlaceholderText(placeholder_example("Customer dimension"))
            elif canonical in ("moduleId", "module_id"):
                value_edit.setPlaceholderText(placeholder_example("12"))
            elif canonical in ("moduleName", "module_name"):
                value_edit.setPlaceholderText(placeholder_example("Data Ingestion"))
            elif canonical in ("objectId", "id"):
                value_edit.setPlaceholderText(placeholder_example("34"))
        ...
        for idx, (label_text, keys) in enumerate(_OBJECT_COL1):
            add_field(0, idx, label_text, keys)
        for idx, (label_text, keys) in enumerate(_OBJECT_COL2):
        ...
        grid.addWidget(labeled_field_block(status_lbl, status_combo), len(_OBJECT_COL1), 0)
        ...
        btn_row = max(len(_OBJECT_COL1) + 1, len(_OBJECT_COL2))
    ...
    def _apply_status_from_record(self) -> None:
        if self._status_combo is None:
            return
        seq = _status_seq_from_record(self._object)
        if seq is None:
            reset_searchable_combo(self._status_combo)
            return
        set_searchable_combo_by_user_data(self._status_combo, seq)

    def set_object(self, obj: dict[str, Any] | None, *, edit_mode: bool = False) -> None:
        self._object = dict(obj) if obj else {}
        self._populate_status_combo()
        self._refresh_values()
        if edit_mode and self._object:
            self._handle_edit()
        else:
            self._switch_to_view_mode()

    def _refresh_values(self) -> None:
        for _label_text, keys in _OBJECT_FIELD_GROUPS:
            canonical = keys[0]
            value = _get_value(self._object, keys)
    ...
    def _has_unsaved_changes(self) -> bool:
        for _label_text, keys in _OBJECT_FIELD_GROUPS:
            ...
            if canonical not in self._editable_keys:
                continue
            original = _get_value(self._object, keys)
        ...
        prev = _status_seq_from_record(self._object)
    ...
    def _handle_save(self) -> None:
        ...
        if not self._has_unsaved_changes():
            ...
            return
        name = self._get_edit_value("objectName", "name")
        if not name:
            self._show_error("Object name is required.")
            name_edit = self._field_edits.get("objectName")
            if name_edit:
                name_edit.setFocus()
            return
        oid = _get_object_id(self._object)
        if oid is None:
            self._show_error("Object ID is missing.")
            return
        ...
        token = self._token()
        result = api_update_object(oid, name, status=status_id, token=token)
        if not result.get("success"):
            self._show_error(str(result.get("message") or "Failed to update object."))
            return

        self._object["objectName"] = name
        self._object["name"] = name
        self._object["status"] = status_id
        self._show_success(str(result.get("message") or "Object updated successfully."))
